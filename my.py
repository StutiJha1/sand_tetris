...
# Move tetromino down by one step (gravity)
...
def sand_settle(grid, current_tetromino, tetromino_position):
    ...
    # Initially place the tetromino in the grid
    for i in range(r):
        for j in range(c):
            if current_tetromino[i][j] == 1:
                grid[row_pos + i][col_pos + j] = 1

    # Now "settle" the tetromino into the grid (like sand filling the gaps)
    for i in range(ROWS-1, -1, -1):
        for j in range(COLUMNS):
            if grid[i][j] == 1:  # Found a block
                # Try to "sink" it down to the lowest position
                k = i
    ...
    # Check if the grid is full at the top (game over condition)
    if any(grid[0] == 1):
...
def game_loop():
    ...
    while True:
        ...
        plt.draw()
        plt.pause(0.5)  # Gravity speed: how fast the tetromino falls (adjust this value)
